...
import { DAI, ETH, USDC, WBTC, anon, deployer, executor, frontendSwapRouterFixtures, swapRouterFixtures } from "./shared/fixtures";
...
describe("FrontendSwapRouterV1", () => {
    let frontendSwapRouterV1: Contract;
    ...
    before(async () => {
        frontendSwapRouterV1 = await frontendSwapRouterFixtures();
        frontendSwapRouterV1.connect(executor);
        ...
        await executor.execute(
            [
                // Transfer in token
                {
                    ...
                    entrypoint: "transfer",
                    calldata: CallData.compile({ recipient: frontendSwapRouterV1.address, amount: { low: 10e18.toString(), high: "0" } })
                },
                // Set swap methods
                {
                    contractAddress: frontendSwapRouterV1.address,
                    entrypoint: "swap",
        ...
        await executor.execute(
            [
                // Transfer in token
                {
                    contractAddress: ETH.address,
                    entrypoint: "transfer",
                    calldata: CallData.compile({ recipient: frontendSwapRouterV1.address, amount: { low: 1e18.toString(), high: "0" } })
                },
                // Set swap methods
                {
                    contractAddress: frontendSwapRouterV1.address,
    ...
    it("Should fail to reinitialize", async () => {
        ...
        try {
            await frontendSwapRouterV1.initialize({ parseRequest: false, parseResponse: false, });
        } catch (e) {
            error = e;
        }
    ...
    it("Should fail only admin assertion", async () => {
        frontendSwapRouterV1.connect(anon);
        let error: any;
        const calldata = CallData.compile({
            tokens: [USDC_ADDRESS]
        });
        ...
        try {
            await frontendSwapRouterV1.withdraw(calldata, { parseRequest: false, parseResponse: false });
        } catch (e) {
            error = e;
        }
    ...
    it("Should withdraw fund from contract", async () => {
        ...
        await ETH.transfer(frontendSwapRouterV1.address, { low: 100e18.toString(), high: "0" });
        frontendSwapRouterV1.connect(deployer);
        balanceBefore = await getETHBalance(frontendSwapRouterV1.address);
        const calldata = CallData.compile({
            tokens: [ETH_ADDRESS]
        });
        ...
        const balanceAfter = await getETHBalance(frontendSwapRouterV1.address);
        expect(balanceAfter).to.be.lessThan(balanceBefore);
